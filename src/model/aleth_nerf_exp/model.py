...
import src.model.aleth_nerf_exp.helper as helper
...
@gin.configurable()
class Aleth_NeRF_Exp(nn.Module):
    def __init__(
        self,
        K_g: float = 1.0,   # Initial Global Density Value
        num_levels: int = 2,    
        min_deg_point: int = 0,
        ...
        super(Aleth_NeRF_Exp, self).__init__()
    ...
    def forward(self, rays, randomized, white_bkgd, near, far, mode): # white_bkgd: False; near: None; far: None
        ...
        for i_level in range(self.num_levels):
            ...
            # Volumn Rendering in Testing Stage, with concealing fileds
            else:   
                comp_rgb_dark, depth, weights = helper.volumetric_rendering(
                    rgb,    # [B, 65/129, 3]
                    sigma,  # [B, 65/129, 1]
                    darkness,   # [B, 65/129, 1]
                    t_vals, 
                ...
                ret.append((comp_rgb_dark, depth, sigma, darkness))

        return ret


@gin.configurable()
class LitAleth_NeRF_Exp(LitModel):
    def __init__(
        self,
        mode: str = 'overexp',       # 'dark' or 'overexp' condition
        eta: float = 0.5,         # Enhance Lightness Degree
        con: float = 0.5,         # Enhance Contrast Degree
        ...
        super(LitAleth_NeRF_Exp, self).__init__()
        self.model = Aleth_NeRF_Exp() # put Aleth-NERF on the optimizer
    ...
    def training_step(self, batch, batch_idx):
        ...
        rgb_coarse_dark, rgb_fine_dark = rendered_results[0][0], rendered_results[1][0]     # normal light RGB
        rgb_coarse_light, rgb_fine_light = rendered_results[0][-1], rendered_results[1][-1] # high light RGB
        
        target = batch["target"]
        
        mean_rgb_coarse = torch.mean(rgb_coarse_dark, dim=0)
        mean_rgb_fine = torch.mean(rgb_fine_dark, dim=0)
        
        # NeRF Loss
        loss0 = helper.img2mse_tone(rgb_coarse_light, target)
        loss1 = helper.img2mse_tone(rgb_fine_light, target)
        # Local Conceiling Control Loss
        
        loss_control = helper.Exp_loss_global(mean_val=self.eta)(rgb_coarse_dark) + helper.Exp_loss_global(mean_val=self.eta)(rgb_fine_dark)   
        target = target.type(torch.FloatTensor).to(rgb_fine_dark.device)
        loss_structure = helper.Structure_Loss(contrast=self.con)(rgb_coarse_dark, target) + helper.Structure_Loss(contrast=self.con)(rgb_fine_dark, target)
        loss_cc = helper.colour(mean_rgb_coarse) + helper.colour(mean_rgb_fine)
        
        loss = loss1 + loss0 + 1e-3*loss_control + 1e-3*loss_structure + 1e-8*loss_cc      # exposure
        ...
        self.log("train/loss_cc", 1e-5*loss_cc, on_step=True)
        self.log("train/loss_control", 1e-3*loss_control, on_step=True)
        self.log("train/loss_structure", 10*loss_structure, on_step=True)
    ...
    def render_rays(self, batch, batch_idx):
        ret = {}
        rendered_results = self.model(
            batch, False, self.white_bkgd, self.near, self.far, mode = 'train'
        )
